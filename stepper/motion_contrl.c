...
void arc_move(struct move_state* ms, union stepper_port* port, uint8_t* out_buffer){
    ...
    while(offset < N-2){
        if(!ms->line_active){
            ...
            dx = (int16_t)((mul_i32_q15((x_new - ms->x), ms->e1[0]) 
                + mul_i32_q15((y_new - ms->y),ms->e2[0])) >> Q15_BITS);

            dy = (int16_t)((mul_i32_q15((x_new - ms->x), ms->e1[1]) 
                + mul_i32_q15((y_new - ms->y),ms->e2[1])) >> Q15_BITS);
...
void motion_control_create(task_t** self){
...
static void motion_control_init(task_t* const super, event_t const* const ie){
    struct motion_control* self = container_of(super, struct motion_control, super); 
...
static void motion_control_dispatch(task_t* const super, event_t* const e){
    ...
    switch (e->signal) {
        ////////////////////////////////////////////////////////
        case EV_WORK_SIG:{
            struct motion_control_workEvt* event = (struct motion_control_workEvt*)e;
